...
const zoomOut={
 ...
 const TrendingItem=({activeItem,item})=>{
  ...
  return (
    ...
    className="w-52 h-72 rounded-[35px] mt-3 bg-white/10" resizeMode={ResizeMode.CONTAIN}
    useNativeControls shouldPlay onPlaybackStatusUpdate={(status)=>{
...
const Trending = ({posts}) => {
  ...
  return (
        <FlatList
          data={posts}
          keyExtractor={(post) => post.$id} // Convert id to string for the keyExtractor
          renderItem={({ item }) => (
            <TrendingItem activeItem={activeItem} item={item}/> // Deconstruct item, not post
          )}
          onViewableItemsChange={viewableItemsChange}
